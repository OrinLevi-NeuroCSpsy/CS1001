...
1. sorting → search_binary
2. higher_order_functions → iterators_generators → memoization

### שלב 3: מבני נתונים

1. oop → linked_lists → hash_tables

### שלב 4: ייצוג מידע

1. number_representation → text_representation
2. compression → error_correction → cryptography

### שלב 5: נושאים נוספים

1. string_matching → image_processing → file_io
...
- [Colab – פתיחת מחברת (דוגמה: חיפוש בינארי)](https://colab.research.google.com/github/orinlevi/CS1001_sum/blob/master/notebooks/binary_search_and_complexity.ipynb)
